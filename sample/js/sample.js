...
let app = new PIXI.Application({
    ...
    backgroundColor: 0x008000,  // 背景色 16進 0xRRGGBB
    autoDensity: true,
});
...
console.log(el)
console.log(app)
el.appendChild(app.view); //view canvas
...
// 大きさを変えてみる
butaSprite2.scale.x = 2;
butaSprite2.scale.y = 2;
// 半透明にしてみる
...
// 色味を変えてみる
butaSprite2.tint = 0x000000;                // (基準は0xffffff)
...
// 画像を読み込み、テクスチャにする
let suikaTexture = new PIXI.Texture.from('./img/suika.png');
// 読み込んだテクスチャから、スプライトを生成する
let suikaSprite = new PIXI.Sprite(suikaTexture);
// ぶたの基準点を設定(%) 0.5はそれぞれの中心 位置・回転の基準になる
suikaSprite.anchor.y = 0.5;
suikaSprite.anchor.x = 0.5;
// ぶたの位置決め
suikaSprite.x = app.screen.width / 2;        // ビューの幅 / 2 = x中央
suikaSprite.y = app.screen.height / 2;       // ビューの高さ / 2 = y中央
// 表示領域に追加する
app.stage.addChild(suikaSprite);

//大きさを変える
suikaSprite.scale.x = 0.25;
suikaSprite.scale.y = 0.25;
...
let polygon = new PIXI.Graphics()
    .beginTextureFill(suikaTexture, 0xffffff, 1, new 0.8)    // 第二引数で透明度を指定できる
